...
TEST(ConcurrentQueueTest, ConcurrentAccess)
{
    ...
    std::atomic<int> counter(0); // Contatore atomico per tenere traccia degli elementi inseriti
    ...
    std::thread consumer([&queue, &counter]() {
        while (counter < num_elements); // Attendere finché tutti gli elementi non sono stati inseriti
        for (int i = 0; i < num_elements; ++i) {
