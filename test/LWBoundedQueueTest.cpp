...
TEST(LWBoundedQueueTest, DequeueEmptyQueue)
{
    ...
    // Attendiamo un breve periodo di tempo per assicurarci che il thread sia bloccato
...
TEST(LWBoundedQueueTest, FillToCapacity)
{
    ...
    producer.joinable() ? producer.join() : std::this_thread::sleep_for(std::chrono::milliseconds(1000));

    ASSERT_TRUE(enqueueFinished); // Verifichiamo se l'inserimento è stato completato
