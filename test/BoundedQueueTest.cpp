...
TEST(BoundedQueueTest, FillToCapacity) {
  ...
  ASSERT_THROW(queue.enqueue(4),
               std::runtime_error);  // Tentativo di inserire un elemento quando
                                     // la coda è piena
}

// Test per verificare che il tentativo di inserire in una coda piena blocchi
// correttamente
TEST(BoundedQueueTest, EnqueueBlockedWhenFull) {
  ...
  queue.enqueue(2);
  std::thread t([&queue]() {
    ASSERT_THROW(queue.enqueue(3),
                 std::runtime_error);  // Tentativo di inserire un elemento
                                       // quando la coda è piena
  });
  std::this_thread::sleep_for(std::chrono::milliseconds(
      100));  // Attendi un breve periodo per assicurarti che il thread t sia in
              // attesa
...
TEST(BoundedQueueTest, DequeueBlockedWhenEmpty) {
  BoundedQueue<int> queue(3);
  std::thread t([&queue]() {
    ASSERT_THROW(queue.dequeue(),
                 std::runtime_error);  // Tentativo di recuperare un elemento
