...
// Test per verificare che il processore elabori correttamente gli elementi dalla coda in modo asincrono
TEST(AsyncQueueProcessorTest, ProcessItems)
{
    ...
    // Attendiamo un po' per assicurarci che il processore abbia elaborato tutti gli elementi
    std::this_thread::sleep_for(std::chrono::milliseconds(100));

    // Il processore dovrebbe aver elaborato tutti gli elementi, quindi la coda dovrebbe essere vuota
    ASSERT_EQ(queue->size(), 0);
}

// Test per verificare che il processore si fermi correttamente quando viene richiesto
TEST(AsyncQueueProcessorTest, StopProcessor)
{
    ...
    // Attendiamo un po' per assicurarci che il processore abbia terminato l'esecuzione
    std::this_thread::sleep_for(std::chrono::milliseconds(100));

    // La coda non dovrebbe essere stata elaborata, quindi la sua dimensione dovrebbe essere ancora 0
    ASSERT_EQ(queue->size(), 0);
}